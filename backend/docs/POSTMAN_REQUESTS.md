...
## ⏰ Управление временными промежутками клиентов

### 15. Обновление временных промежутков клиента
```
PUT http://localhost:8000/api/jacobs/dataset/clients/{client_number}/time
Authorization: Bearer YOUR_ACCESS_TOKEN
Content-Type: application/json
...
**Параметры:**
- `client_number` - номер клиента (4-значный)
- `start` - время начала промежутка (формат: "HH:MM", например "09:00")
- `end` - время окончания промежутка (формат: "HH:MM", например "17:00")

**Примеры запросов:**

Обновить только время начала:
```json
{
    "start": "08:30"
}
...
Обновить только время окончания:
```json
{
    "end": "18:00"
}
...
**Примечания:**
- Поля `start` и `end` изначально имеют значение `null` при создании клиента
- Времена должны быть в том же формате, что и времена в датасете (например, "09:00", "17:30")
- Можно обновлять как оба поля одновременно, так и по отдельности
- Поля являются опциональными - можно передать только `start` или только `end`
